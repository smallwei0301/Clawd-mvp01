...
def find_bbox(img: Image.Image) -> tuple[int, int, int, int]:
    ...
    # ignore left nav; start search a bit to the right
    x0, y0 = int(w * 0.18), int(h * 0.12)
    sub = arr[y0:, x0:]

    # background is near-white in Gemini UI
...
def main():
    ...
    src = Path(r"C:\Users\KN222\.clawdbot\media\browser\bef43753-8297-45d5-8ca1-f50d0dbc76a6.png")
    if not src.exists():
    ...
    # save as the new input
    out = outdir / "input_bottle_scene.jpg"
